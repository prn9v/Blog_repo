...
const UpdateBlog = () => {
	...
	useEffect(() => {
		const fetchBlog = async () => {
			...
			try {
				setLoading(true)
				const response = await fetch(`${process.env.NEXT_PUBLIC_BACKEND_URL}/v1/teams/blogs/${params.id}`, {
					credentials: 'include'
				})
	...
	const handlePublish = async () => {
		...
		try {
			...
			if (!publishResponse.ok) {
				...
				throw new Error(errorData.message || `HTTP error! Status: ${publishResponse.status}`)
			}
			...
			<div className='container max-w-4xl mx-auto px-4 space-y-6'>
				<div className='flex items-center gap-4 mb-4'>
					...
					<div className='flex items-center gap-3'>
						...
						<div>
							...
							<p className='text-sm text-muted-foreground'>Edit your blog post and save changes ✍️</p>
						</div>
					</div>
				</div>
				...
				<form className='space-y-6'>
								...
								<div className='flex flex-wrap gap-2 mt-2'>
									{formData.tags.map((tag, index) => (
										<Badge
											...
											onClick={() =>
												removeTag(typeof tag === 'object' ? tag.word || tag.title || '' : tag)
											}
										>
					...
					<div className='flex justify-end gap-4'>
						...
						<Button type='button' onClick={handleSave} disabled={isSaving || isPublishing || loading}>
							{isSaving ? (
								<>
